...
class Config(object):
    def __init__(self, instance_name, infer=False, infer_path=""):
        ...
        if os.path.exists("save/" + instance_name +"/conf.pkl"):
            ...
            with open(os.path.join(self.save_dir, "conf.pkl"),"w") as f:
                cPickle.dump(self, f) 
            self.nerloader = NerLoader(self.save_dir,self.data_dir,self.batch_size,self.seq_length,self.instance_name, infer)
